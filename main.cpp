/*
 Partially based on https://github.com/Tarik02/machineid
*/
...
#include <tchar.h>
    ...
    unsigned short hashMacAddress(PIP_ADAPTER_INFO info) {
        ...
        for (unsigned short i = 0; i < info->AddressLength; i++) {
        ...
        for (ifap = ifaphead; ifap; ifap = ifap->ifa_next)
        {
            ...
            if (sdl && (sdl->sdl_family == AF_LINK) && (sdl->sdl_type == IFT_ETHER))
            {
                if (!foundMac1)
                {
                    foundMac1 = true;
                    mac1 = hashMacAdd
ress((unsigned char*)(LLADDR(sdl))); //sdl->sdl_data) + sdl->sdl_nlen) );
                }
                else {
    ...
    std::string getCpuHash() {
        CPUInfo cinfo;
        std::ostringstream s;
        s << cinfo.vendor() << cinfo.model() << cinfo.cores() << cinfo.logicalCpus() << cinfo.isHyperThreaded() << cinfo.isSSE() << cinfo.isSSE2() << cinfo.isSSE3() << cinfo.isSSE41() << cinfo.isSSE42() << cinfo.isAVX();
        return generateHash(s.str());
    }
#endif // DARWIN
...
int main()
{
    ...
    //file.open("info.txt");
    unsigned short mac1, mac2;
    machineid::getMacHash(mac1, mac2);
    cout << std::string(machineid::getMachineName());
    cout << ",";
    cout << machineid::getCpuHash();
    cout << ",";
    cout << machineid::getVolumeHash();
    cout << ",";
    cout  << mac1 << mac2;
    cout << ",";
    cout << machineid::machineHash() << endl;
    //file.close();
    //cout << machineid::machineHash() << endl;
    //cout << "Se ha recuperado satisfactoriamente la información necesaria y se ha guardado en el fichero info.txt localizado en la misma ruta que este ejecutable." << endl;
    return 0;
}
